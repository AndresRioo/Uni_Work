...
public class Client {
    ...
    int port;
    ComUtils comutils;
    BattleshipComUtils battleshipComUtils;
    ...
    /**
     * Returns the BattleShipComUtils instance for communication.
     * If not initialized, it initializes it first.
     *
     * @return The BattleShipComUtils instance.
     */
    public BattleshipComUtils getBSComutils() {
        if (battleshipComUtils == null) {
            try {
                battleshipComUtils = new BattleshipComUtils(socket.getInputStream(), socket.getOutputStream());
            } catch (IOException e) {
                throw new RuntimeException("I/O Error when creating the ComUtils:\n"+e.getMessage());
            }
        }
        return battleshipComUtils;
    }

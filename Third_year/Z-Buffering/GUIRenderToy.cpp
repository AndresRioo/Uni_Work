...
void GUIRenderToy::renderControls(GLWidget& glWidget) {
    ...
    bool mappingIndirecte = setup.mappingIndirecte;
    bool changeLastObjectTexture = setup.changeLastObjectTexture;

    // GPUMaterial controls
    if (ImGui::Checkbox("Mapping Indirecte", &mappingIndirecte)) {
        setup.mappingIndirecte = mappingIndirecte;
    }

    if (ImGui::Checkbox("Canviar textura només ultim objecte", &changeLastObjectTexture)) {
        setup.changeLastObjectTexture = changeLastObjectTexture;
    }

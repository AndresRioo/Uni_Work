...
public:
    examen(shared_ptr<Scene> scene, vector<shared_ptr<Light>> lights, shared_ptr<Camera> camera, shared_ptr<Config> config , shared_ptr<ShadingStrategy> shadingStrategy) {
        this->scene = scene;
        this->lights = lights;
        this->camera = camera;
        this->config = config;

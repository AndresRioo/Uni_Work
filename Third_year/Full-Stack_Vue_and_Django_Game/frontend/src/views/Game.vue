...
onMounted(() => {
  const storedGameId = false; // sessionStorage.getItem("currentGameId");
  if (storedGameId) {
    store.getGameState(storedGameId);
  } else {
    store.startNewGame();  // solo si no hay gameId
  }
});
    ...
    <div class="row">
      ...
      <div v-else class="col-lg-2 d-flex flex-column justify-content-center">
        <div class="game-controls text-center">
          ...
          <button class="btn btn-primary" @click="store.startNewGameFromPlayingGame()">
            New Game
          </button>
        </div>
      </div>

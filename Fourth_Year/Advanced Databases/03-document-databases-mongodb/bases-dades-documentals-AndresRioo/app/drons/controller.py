...
def get_leaderboard(limit: int = 10, db: Session = Depends(get_db)):
    return repository.get_leaderboard(db, limit)

@router.get("/search")
def search_drons(longitude: float, latitude: float, radius: float = 10.0, db: Session = Depends(get_db)):
    return repository.search_by_location(db, longitude, latitude, radius)
